class Profile {
  ...
  Profile({required this.name, required this.avatarUrl, required this.ageDescription, this.bloodType});

  factory Profile.fromDb({required Map<String, dynamic> profileRow, required Map<String, dynamic> childRow}) {
    // Prefer baby (child) details for Home profile card using schema fields
    final name = (childRow['first_name'] as String?)
    ...
    return Profile(name: name, avatarUrl: avatar, ageDescription: ageDesc, bloodType: bloodType);
  }
}
...
class ScheduleEntry {
  ...
  ScheduleEntry({required this.id, required this.title, required this.time, required this.icon, required this.status, this.isoDate});
}
...
class HomeData {
  ...
  HomeData({required this.profile, required this.stats, required this.logs, required this.schedule, required this.memories, this.growthSummary});
}
...
class GrowthSummary {
  ...
  final String date; // display string
  GrowthSummary({this.height, this.weight, this.headCircumference, required this.date});
}
