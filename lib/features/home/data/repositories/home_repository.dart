...
class SupabaseHomeRepository implements HomeRepository {
  ...
  SupabaseHomeRepository({SupabaseClient? client}) : client = client ?? Supabase.instance.client;
  ...
  Future<HomeData> getHomeDataForCurrentUser({String? parentId}) async {
    ...
    final childResp = await client
      ...
      .limit(1)
      .maybeSingle();
    if (childResp == null) throw Exception('No child linked to this user');

    final profile = Profile.fromDb(profileRow: profileResp, childRow: childResp);
    ...
    final checklist = await client
      .from('checklist')
      .select('id, title, done, created_at, updated_at, checklist_date, child_id, user_id')
      .eq('child_id', childId)
      .eq('user_id', userId)
    ...
    if (filtered.isEmpty) {
      final checklistChildOnly = await client
          ...
          .order('created_at', ascending: false);
      // ignore: avoid_print
      print('[home] checklist rows (child only) => ${checklistChildOnly.length}');
      filtered = checklistChildOnly.whereType<Map>().toList();
    }

    String _inferType(Map item) {
      final explicit = (item['type'] as String?)?.toLowerCase();
      if (explicit != null && explicit.isNotEmpty) return explicit;
      final title = (item['title'] as String?)?.toLowerCase() ?? '';
      if (title.contains('diaper')) return 'diaper';
      if (title.contains('temp') || title.contains('temperature')) return 'temperature';
      if (title.contains('feed') || title.contains('bottle') || title.contains('milk')) return 'feeding';
    ...
    for (final item in filtered) {
      ...
      logs.add(LogEntry(id: item['id'].toString(), type: type, value: title, timestamp: ts));
    ...
    var growthRows = await client
      .from('growth')
      .select('date, height, weight, head_circumference, unit_height, unit_weight, unit_head_circ')
      .eq('child_id', childId)
      .eq('user_id', userId)
    ...
    if (growthRows.isNotEmpty) {
      ...
      growthSummary = GrowthSummary(
        height: (r['height'] != null ? '${r['height']} ${r['unit_height'] ?? ''}'.trim() : null),
        weight: (r['weight'] != null ? '${r['weight']} ${r['unit_weight'] ?? ''}'.trim() : null),
        headCircumference: (r['head_circumference'] != null ? '${r['head_circumference']} ${r['unit_head_circ'] ?? ''}'.trim() : null),
        date: (r['date']?.toString()) ?? '',
      );
    } else {
      // Ensure GrowthSection shows placeholders even without data
      growthSummary = GrowthSummary(height: null, weight: null, headCircumference: null, date: '');
    }
    ...
    final stats = <Stat>[
      Stat(title: 'Sleep', subtitle: 'Avg. Sleep', value: avgSleep, icon: 'time'),
      Stat(title: 'Feeding', subtitle: 'Avg. Feeding', value: feedingsPerDay, icon: 'feeding'),
      if (profile.bloodType != null && profile.bloodType!.isNotEmpty)
        Stat(title: 'Blood', subtitle: 'Blood Type', value: profile.bloodType!, icon: 'hospital'),
      if (growthSummary.weight != null && growthSummary.weight!.isNotEmpty)
        Stat(title: 'Weight', subtitle: 'Latest', value: growthSummary.weight!, icon: 'feeding'),
    ];
  ...
  @override
  Future<HomeData> getHomeDataForChild(String childId, {String? parentId}) async {
    ...
    final childResp = await client
        ...
        .maybeSingle();
    if (childResp == null) {
      // Improve diagnostics if RLS blocks or row missing
      final uid = client.auth.currentUser?.id;
      throw Exception('Child not found. Tips: verify children.id=$childId exists, that its parent_id matches auth.uid=$uid or provided parentId=$userId, and that RLS SELECT policy allows this read.');
    }

    final profile = Profile.fromDb(profileRow: profileResp ?? <String, dynamic>{}, childRow: childResp);
  ...
  static Future<String> _computeAverageSleep(SupabaseClient client, String childId) async {
    ...
    final avgMinutes = durations.map((d) => d.inMinutes).reduce((a, b) => a + b) / durations.length;
    final h = (avgMinutes ~/ 60);
    final m = (avgMinutes % 60).round();
    return '${h}h ${m}m';
  }

  static Future<String> _computeFeedingPerDay(SupabaseClient client, String childId) async {
    final rows = await client
        .from('meals')
        .select('meal_time')
        .eq('child_id', childId)
