...
class GalleryState {
  ...
  factory GalleryState.initial() =>
      GalleryState(loading: false, items: []);
}
...
class GalleryCubit extends Cubit<GalleryState> {
  ...
  Future<void> addMemory({
    ...
    try {
      ...
      emit(GalleryState(loading: false, items: state.items, error: 'Failed to add memory: $e'));
    }
  }
  ...
  Future<void> updateMemory({
    ...
    try {
      ...
      final updatedList = state.items.map((e) => e.id == id ? updatedItem : e).toList();
      emit(GalleryState(loading: false, items: updatedList));
    } catch (e) {
      emit(GalleryState(loading: false, items: state.items, error: 'Failed to update memory: $e'));
    }
  }
  ...
  Future<void> deleteMemory(GalleryItem item) async {
    ...
    try {
      ...
      emit(GalleryState(loading: false, items: state.items, error: 'Failed to delete memory: $e'));
    }
  }
}
