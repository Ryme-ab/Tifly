  ...
  Widget build(BuildContext context) {
    return Provider<AppBarConfig>(
      ...
      child: Scaffold(
          ...
          return Padding(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                if (index == 0) {
                  return _AddTile(onTap: () async {
                    final res = await Navigator.of(context).push<bool>(
                      MaterialPageRoute(
                        builder: (_) => AddMemoryPage(childId: widget.childId, userId: widget.userId),
                      ),
                    );
                ...
                return _GalleryTile(
                  item: item,
                  onTap: () {
                    showDialog(context: context, barrierDismissible: false, builder: (_) => MemoryModal(memory: item));
                  },
                  onEdit: () async {
                    final res = await Navigator.of(context).push<bool>(
                      MaterialPageRoute(
                        builder: (_) => AddMemoryPage(childId: widget.childId, userId: widget.userId, existing: item),
                      ),
                    );
                    if (res == true) _cubit.loadGallery(widget.childId);
                  },
        ...
        onPressed: () async {
          final res = await Navigator.of(context).push<bool>(
            MaterialPageRoute(builder: (_) => AddMemoryPage(childId: widget.childId, userId: widget.userId)),
          );
          if (res == true) _cubit.loadGallery(widget.childId);
        },
...
class _AddTile extends StatelessWidget {
  ...
  const _AddTile({required this.onTap, Key? key}) : super(key: key);
...
class _GalleryTile extends StatelessWidget {
  ...
  const _GalleryTile({required this.item, required this.onTap, this.onEdit, this.onDelete, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final dateText = item.pictureDate != null ? DateFormat.yMMMd().format(item.pictureDate!) : '';
    return GestureDetector(
      onTap: onTap,
      child: ClipRRect(
        borderRadius: BorderRadius.circular(12),
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Text(item.title ?? '', style: AppFonts.body.copyWith(color: Colors.white, fontWeight: FontWeight.w700)),
                  if (dateText.isNotEmpty)
                    Text(dateText, style: AppFonts.small.copyWith(color: Colors.white70)),
                ],
              ),
            ),
          ),
