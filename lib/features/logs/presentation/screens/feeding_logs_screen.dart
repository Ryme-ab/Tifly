...
class _FeedingLogsScreenState extends State<FeedingLogsScreen> {
  ...
  Future<void> _exportFeedingLogsPDF(BuildContext context) async {
    final state = context.read<FeedingLogCubit>().state;
    ...
    try {
        ...
        pw.MultiPage(
          ...
          build: (context) => [
            ...
            pw.Table.fromTextArray(
              ...
              data: logs.map((log) => [
                DateFormat('MMM dd, hh:mm a').format(log.mealTime),
                log.mealType,
                log.items,
                '${log.amount}ml',
              ]).toList(),
              headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.white),
              headerDecoration: const pw.BoxDecoration(color: PdfColors.blue500),
      ...
      await Printing.layoutPdf(
        ...
        name: 'feeding_logs_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.picture_as_pdf, color: Color(0xFFF56587)),
            onPressed: () => _exportFeedingLogsPDF(context),
            tooltip: 'Export PDF',
          ),
        ],
      ),
