...
class _GrowthLogsScreenState extends State<GrowthLogsScreen> {
  ...
  Future<void> _exportGrowthLogsPDF(BuildContext context) async {
    final state = context.read<GrowthLogCubit>().state;
    if (state is! GrowthLogLoaded) return;

    final logs = _filterLogs(state.logs);
    if (logs.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No growth logs to export')),
      );
      return;
    }
    ...
    try {
          ...
          build: (context) => [
            pw.Header(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text('Growth Logs Report', style: pw.TextStyle(fontSize: 28, fontWeight: pw.FontWeight.bold, color: PdfColors.purple700)),
            ...
            pw.Table.fromTextArray(
              headers: ['Date', 'Height (cm)', 'Weight (kg)', 'Head Circ. (cm)', 'Notes'],
              data: logs.map((log) {
                return [
                  DateFormat('MMM dd, yyyy').format(log.date),
                  log.height?.toStringAsFixed(1) ?? '-',
                  log.weight?.toStringAsFixed(2) ?? '-',
                  log.headCircumference?.toStringAsFixed(1) ?? '-',
                  log.notes?.isEmpty ?? true ? '-' : log.notes!,
                ];
              }).toList(),
              headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.white),
              headerDecoration: const pw.BoxDecoration(color: PdfColors.purple500),
      ...
      await Printing.layoutPdf(
        ...
        name: 'growth_logs_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(
        title: l10n.growthTracker,
        actions: [
          IconButton(
            icon: const Icon(Icons.picture_as_pdf, color: Color(0xFF8E44AD)),
            onPressed: () => _exportGrowthLogsPDF(context),
            tooltip: 'Export PDF',
          ),
        ],
      ),
